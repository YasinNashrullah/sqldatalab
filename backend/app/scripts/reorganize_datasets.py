    ...
    for i in range(1, 81):
        ...
        orders_rows.append(
            [
                ...
                cid,
                pid,
                f"2026-0{random.randint(1, 2)}-{random.randint(1, 28):02d}",
                qty,
                total,
...
def generate_fintech():
    ...
    for aid in acc_ids:
        accounts_rows.append(
            [
                ...
                f"Customer {aid[-3:]}",
                random.choice(types),
                random.randint(500000, 85000000),
                "Active",
                f"2024-{random.randint(1, 12):02d}-{random.randint(1, 28):02d}",
            ]
        )
    write_csv(
        banking_dir / "accounts.csv",
        [
            "account_id",
            "customer_name",
            "account_type",
    ...
    write_csv(
        banking_dir / "merchants.csv",
        ["merchant_id", "merchant_name", "category", "city", "fee_rate"],
        merchants_rows,
    )
    ...
    for i in range(1, 85):
        ...
        tx_rows.append(
            [
                ...
                aid,
                mid,
                f"2026-02-{random.randint(1, 28):02d} {random.randint(8, 21):02d}:{random.randint(10, 59):02d}:00",
                random.choice(tx_types),
                amount,
                fee,
                "SUCCESS" if is_fraud == "NO" else "FLAGGED_FRAUD",
                is_fraud,
            ]
        )
    write_csv(
        banking_dir / "transactions.csv",
        [
            "transaction_id",
            "account_id",
            "merchant_id",
...
def generate_healthcare():
    ...
    write_csv(
        hosp_dir / "doctors.csv",
        ["doctor_id", "doctor_name", "specialization", "department", "room_number"],
        docs_rows,
    )
    ...
    for pid in p_ids:
        patients_rows.append(
            [
                ...
                f"0812{random.randint(10000000, 99999999)}",
            ]
        )
    write_csv(
    ...
    for i in range(1, 71):
        ...
        admissions_rows.append(
            [
                ...
                pid,
                did,
                f"2026-01-{random.randint(1, 20):02d}",
                f"2026-01-{random.randint(21, 28):02d}",
                random.choice(diagnoses),
                days,
                cost,
                covered,
                "BPJS" if covered > 0 else "Mandiri",
            ]
        )
    write_csv(
...
def generate_hr():
    ...
    for emp in emp_rows:
        ...
        salaries_rows.append(
            [f"SAL-{eid}", eid, base, bonus, deductions, net, "Monthly"]
        )
    write_csv(
        hr_dir / "salaries.csv",
        [
...
def generate_logistics():
    ...
    write_csv(
        log_dir / "warehouses.csv",
        ["hub_id", "hub_name", "city", "province", "capacity_sqm"],
        warehouses_rows,
    )
    ...
    write_csv(
        log_dir / "carriers.csv",
        ["carrier_id", "carrier_name", "service_type", "customer_rating", "hotline"],
        carriers_rows,
    )
    ...
    for i in range(1, 81):
        ...
        shipments_rows.append(
            [
                ...
                est,
                act,
                "Delivered" if act <= est else "Delayed",
                delayed,
            ]
        )
    write_csv(
        log_dir / "shipments.csv",
        [
            "tracking_no",
            "origin_hub_id",
            "dest_hub_id",
...
def generate_education():
    ...
    write_csv(
        edu_dir / "courses.csv",
        ["course_id", "course_name", "credits", "department", "semester_offered"],
        courses_rows,
    )
...
def generate_retail():
    ...
    write_csv(
        ret_dir / "suppliers.csv",
        ["supplier_id", "supplier_name", "city", "lead_time_days", "vendor_rating"],
        suppliers_rows,
    )
    ...
    write_csv(
        ret_dir / "products.csv",
        ["sku", "product_name", "category", "retail_price_idr"],
        products_rows,
    )
    ...
    for i, p in enumerate(products_rows):
        ...
        inventory_rows.append(
            [
                ...
                reorder,
                cost,
                "Normal" if stock > reorder else "RESTOCK_NEEDED",
            ]
        )
    write_csv(
        ret_dir / "inventory.csv",
        [
            "inventory_id",
            "sku",
            "supplier_id",
    ...
    for i in range(1, 41):
        ...
        guests_rows.append(
            [
                gid,
                f"Guest {i:03d}",
                f"guest{i:03d}@example.com",
                random.choice(countries),
                random.randint(100, 3500),
            ]
        )
    write_csv(
        hotel_dir / "guests.csv",
        ["guest_id", "full_name", "email", "nationality", "loyalty_points"],
        guests_rows,
    )
    ...
    for i in range(1, 65):
        ...
        bookings_rows.append(
            [
                ...
                gid,
                rid,
                f"2026-02-{random.randint(1, 20):02d}",
                nights,
                total,
...
def generate_saas():
    ...
    write_csv(
        saas_dir / "plans.csv",
        ["plan_id", "plan_name", "monthly_price_usd", "seat_limit", "target_audience"],
        plans_rows,
    )
    ...
    for i in range(1, 65):
        ...
        invoices_rows.append(
            [
                ...
                f"2026-02-{random.randint(1, 28):02d}",
                plan_price,
                "PAID" if random.random() > 0.1 else "PENDING",
            ]
        )
    write_csv(
        saas_dir / "invoices.csv",
        [
            "invoice_id",
            "subscription_id",
            "invoice_date",
...
def generate_telecom():
    ...
    write_csv(
        tel_dir / "services.csv",
        ["service_id", "service_name", "category", "monthly_fee_idr"],
        services_rows,
    )
    ...
    for c in cust_rows:
        ...
        churned = (
            "YES" if (contract == "Month-to-Month" and random.random() < 0.45) else "NO"
        )
        risk = round(random.uniform(0.1, 0.95), 2)
        churn_rows.append([f"REC-{cid}", cid, contract, charge, tenure, churned, risk])
    write_csv(
        tel_dir / "churn_records.csv",
        [
            "record_id",
            "customer_id",
            "contract_type",
